...
public ResponseEntity<?> endMatch(@PathVariable Long requestId, Authentication auth) {
    try {
        ...
        if (fromRide.getUserId().equals(user.getId())) {
            ...
            logger.info("User does not own either ride in request - user={}, requestId={}",
                    user.getUsername(), requestId);
            return ResponseEntity.status(HttpStatus.FORBIDDEN).build();
        }

        // Allow end while status is CONFIRMED or COMPLETED (idempotent)
        if (!request.getStatus().equals(RideStatus.CONFIRMED)
                && !request.getStatus().equals(RideStatus.COMPLETED)) {
        ...
        if (bothEnded) {
            ...
            // Mark match request COMPLETED – this is what FeedbackService depends on
            request.setStatus(RideStatus.COMPLETED);
            rideMatchRequestRepository.save(request);

            logger.info("Journey completed for both. requestId={}", requestId);
            return ResponseEntity.ok(
                    Map.of("completedForBoth", true,
                           "message", "Journey completed!",
                           "rideId", userRide.getId())
            );
        } else {
            logger.info("End confirmed by one user. Waiting for partner. requestId={}", requestId);
            return ResponseEntity.ok(
                    Map.of("completedForBoth", false,
